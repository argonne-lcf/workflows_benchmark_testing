...
class GNN(nn.Module):
    def __init__(self, train_args, input_channels, spatial_dim):
        """
    ...
    def build_model(self) -> nn.Module:
        ...
        :return: GNN model
        """
        input_edge_channels = self.input_channels + self.spatial_dim + 1
        model = MP_GNN(input_node_channels = self.input_channels,
                       input_edge_channels = input_edge_channels,
                       hidden_channels = self.cfg.hidden_channels,
                       output_node_channels = self.output_channels,
                       n_mlp_hidden_layers = self.cfg.mlp_hidden_layers,
                       n_messagePassing_layers = self.cfg.message_passing_layers)
        return model
    
    def setup_local_graph(self, pos, ei):
        """
