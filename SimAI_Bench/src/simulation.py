...
def main():
    ...
    if rank==0:
        logger.info(f'Running {args.problem_size} problem size, ')
    ...
    with Stream(io, 'problem_definition', 'w', comm) as stream:
        ...
        stream.write('coords', problem_def['coords'], [arr_size*size], [rank*arr_size], [arr_size])
        arr_size = problem_def['edge_index'].size
        stream.write('edge_index', problem_def['edge_index'], [arr_size*size], [rank*arr_size], [arr_size])
        stream.end_step()
    comm.Barrier()
    ...
    for istep_w in range(args.workflow_steps):
        ...
        for istep_s in range(args.simulation_steps):
            sleep(0.5)
            train_data = simulation_step(args.problem_size, problem_def['coords'])
